...
fn timeSince(time: std.Io.Timestamp, prev_time: std.Io.Timestamp) ?u64 {
    const elapsed = prev_time.durationTo(time).toNanoseconds();
    if (elapsed < 0) return null;
    return @intCast(std.math.clamp(elapsed, 0, std.math.maxInt(u64)));
}
...
fn testTimestamp(ns: u64) std.Io.Timestamp {
...
test "SelectionGesture backwards repeat time resets click count" {
    ...
    const earlier = testTimestamp(std.time.ns_per_s);
    const later = testTimestamp(2 * std.time.ns_per_s);
