...
test "Terminal: scrollUp top region no scrollback" {
    const alloc = testing.allocator;
    var t = try init(testing.io, alloc, .{ .rows = 5, .cols = 5, .max_scrollback = 0 });
    defer t.deinit(alloc);
...
test "Terminal: printSlice wraps and scrolls" {
    const alloc = testing.allocator;
    var t = try init(testing.io, alloc, .{ .cols = 5, .rows = 2 });
    defer t.deinit(alloc);
...
fn testPrintSliceDifferential(
    ...
    var t1 = try init(testing.io, alloc, .{
        .cols = cols,
        .rows = rows,
    });
    defer t1.deinit(alloc);
    var t2 = try init(testing.io, alloc, .{
        .cols = cols,
        .rows = rows,
    });
